...
// Perform tree traversal on Mocha suite. Parent-child tree
const inspectSuite = (suite, dict) => {
  if (suite.suites) {
    const suiteData = getMetadata(suite.suites, "suite")
    dict.set(suite.title, suiteData);
    suite.suites.forEach((suite) => {
      inspectSuite(suite, dict);
    });
  }

  if (suite.tests) {
    const testData = getMetadata(suite.tests, "test")
    dict.get(suite.title).push(testData);
  }
};

const getMetadata = (arr, type) => arr.map(a => ({ title: a.title, id: a.id, type}))
