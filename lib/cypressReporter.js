  ...
  constructor(runner, config) {
    ...
    if (CypressReporter.isFirstRun() || !config.reporterOptions.autoMerge) {
      ...
      startIPCServer(
        ...
        (server) => {
          console.log('Server process ending:', server);
    ...
    this.runner.on(EVENT_SUITE_BEGIN, (suite) => {
      ...
      // If this is the first suite in a file, send the test structure to the Reporter
      if (suite.id === 'r2') {
        inspectSuite(suite, specMap);

        const specObj = Array.from(specMap).reduce((obj, [key, value]) => {
          obj[key] = value.flat();
          return obj;
        }, {});

        this.worker.send({
          event: 'EVENT_SEND_SPECS',
          data: specObj,
          count: CypressReporter.totalLaunches,
        });
      }
